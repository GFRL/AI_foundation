    ...
    std::vector<double> sample_path(const GameState& state, double exploration) {
        ...
        if (node->n_children() < state.n_actions()) {
            ...
            for (int i = 0, child; i < node->n_children(); ++i) {
                ...
                // 选择UCT值最大的子结点继续探索
                double x = value_sums_of[child][state.active_player()] / visit_count_of[child]
                    + exploration * sqrt(log(visit_count_of[index]) / visit_count_of[child]);
                if (x > best_value)best_value = x, best_son = i;
            }

            next_state = state.next(state.action_space()[best_son]);
...
public:
    ...
    ActionType select_action(double exploration) {
        ...
        for (int i = 0, child; i < root->n_children(); ++i) {
            ...
            // 按平均价值贪心选择
            double x = value_sums_of[child][root_state.active_player()] / visit_count_of[child];
            if (x > best_value)best_value = x, best_son = i;
        }

        // 也可以按照访问次数贪心选择

        return root_state.action_space()[best_son];
    }
};
...
Json::Value get_next_action(const HexState<N>& state, bool forced_flag) {
    ...
    int action = mcts.select_action(exploration);
...
int main() {
    ...
    HexState<N> state;
    TIME_LIMIT = CLOCKS_PER_SEC * 95 / 100;
    // 读入JSON
    string str;
    getline(cin, str);
    Json::Reader reader;
    ...
    result["debug"] = "Expand times: " + to_string(expand_times) + " using times: " + to_string(clock() - t0);
    Json::FastWriter writer;
    int tmp1 = result["response"]["x"].asInt(), tmp2 = result["response"]["y"].asInt();
    state = state.next(tmp1 * N + tmp2);
    ...
    while (1) {
        ...
        forced_flag = false;
        reader.parse(str, input);
        int X2 = input["x"].asInt();
        int Y2 = input["y"].asInt();
        state = state.next(X2 * N + Y2);
        t0 = clock();
        result["response"] = get_next_action(state, forced_flag);
        result["debug"] = "Expand times: " + to_string(expand_times) + " using times: " + to_string(clock() - t0);
        X2 = result["response"]["x"].asInt();
        Y2 = result["response"]["y"].asInt();
        state = state.next(X2 * N + Y2);
        cout << writer.write(result) << endl;
        cout << "\n>>>BOTZONE_REQUEST_KEEP_RUNNING<<<" << endl;
    }
